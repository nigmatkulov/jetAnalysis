...
class ForestAODReader : public BaseReader {
  ...
  public:
    ...
    ForestAODReader(const Char_t* inputStream, 
                    const Bool_t& useHltBranch = kTRUE, const Bool_t& usePFJetBranch = kTRUE, 
                    const Bool_t& useCaloJetBranch = kFALSE, const Bool_t& useTrackBranch = kFALSE,
                    const Bool_t& useGenTrackBranch = kFALSE, const Bool_t& isMc = kFALSE );
    /// @brief Destructor
    ...
    /// @brief Set HLT filter names and statuses (branch names and ON/OFF status)
    void setHltFilters(std::list<TString> filtName, std::list<Int_t> filtStatus) 
    { fHltFilterName = filtName; fHltFilterStatus = filtStatus;}
  ...
  private:
    ...
    /// @brief Jet HLT path trigger used for analysis
    Int_t  fJetTriggerBit; 

    /// @brief List of HLT filter names
    std::list<TString> fHltFilterName;
    /// @brief List of HLT filter statuses
    std::list<Int_t>  fHltFilterStatus;
