...
Build the executable before submission:
...
```bash
cp "$(voms-proxy-info -path)" processing/voms_proxy.txt
```
...
py-env/bin/python processing/submit_process_forest_condor.py \
  processing/filelists/pPb8160/DATA_MB/Pbgoing/MB_PD12_Pbgoing.txt \
...
The default proxy is `processing/voms_proxy.txt`; override it with
...
`processing/condor/<campaign-id>/<sample-name>/`; outputs go to the requested
...
`processing/voms_proxy.txt` is used by default. Every generated submit file
contains:
...
py-env/bin/python processing/submit_process_forest_condor.py \
  'processing/filelists/pPb8160/DATA_MB/Pbgoing/MB_PD{pd}_Pbgoing.txt' \
...
py-env/bin/python processing/submit_process_forest_condor.py \
  'processing/filelists/pPb8160/DATA_MB/pgoing/MB_PD{pd}_pgoing.txt' \
...
# Jet60 p-going
py-env/bin/python processing/submit_process_forest_condor.py \
  processing/filelists/pPb8160/DATA_PAEGJet/pgoing/PAEGJet_pgoing.txt \
...
# Embedded Pb-going
py-env/bin/python processing/submit_process_forest_condor.py \
  'processing/filelists/pPb8160/MC_embedded/Pbgoing/MC_pthat{pt_hat}_Pbgoing_embedded.txt' \
  /eos/.../embedding/Pbgoing --mc-type 1 --beam Pbgoing --pt-hat all

# Embedded p-going
py-env/bin/python processing/submit_process_forest_condor.py \
  'processing/filelists/pPb8160/MC_embedded/pgoing/MC_pthat{pt_hat}_pgoing_embedded.txt' \
  /eos/.../embedding/pgoing --mc-type 1 --beam pgoing --pt-hat all

# Pythia Pb-going
py-env/bin/python processing/submit_process_forest_condor.py \
  'processing/filelists/pPb8160/MC_Unembedded/Pbgoing/MC_pthat{pt_hat}_Pbgoing_unembedded.txt' \
  /eos/.../pythia/Pbgoing --mc-type 2 --beam Pbgoing --pt-hat all

# Pythia p-going
py-env/bin/python processing/submit_process_forest_condor.py \
  'processing/filelists/pPb8160/MC_Unembedded/pgoing/MC_pthat{pt_hat}_pgoing_unembedded.txt' \
