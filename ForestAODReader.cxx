...
ForestAODReader::ForestAODReader() : fEvent{nullptr}, fInFileName{nullptr}, fEvents2Read{0}, fEventsProcessed{0},
    fIsMc{kFALSE}, fUseHltBranch{kTRUE}, fUsePartFlowJetBranch{kTRUE},
    fUseCaloJetBranch{kFALSE}, fUseTrackBranch{kFALSE}, fUseGenTrackBranch{kFALSE},
    fHltTree{nullptr}, fEventTree{nullptr}, fCaloJetTree{nullptr},
...
ForestAODReader::ForestAODReader(const Char_t* inputStream,  
                    const Bool_t& useHltBranch, const Bool_t& usePFJetBranch, 
                    const Bool_t& useCaloJetBranch, const Bool_t& useTrackBranch,
                    const Bool_t& useGenTrackBranch, const Bool_t& isMc) : 
    fEvent{nullptr}, fInFileName{inputStream}, fEvents2Read{0}, fEventsProcessed{0}, fIsMc{isMc}, 
    fUseHltBranch{useHltBranch}, fUsePartFlowJetBranch{usePFJetBranch},
    fUseCaloJetBranch{useCaloJetBranch}, fUseTrackBranch{useTrackBranch}, 
    fUseGenTrackBranch{useGenTrackBranch},
    fJEC{nullptr}, fJECFiles{}, fJEU{nullptr}, fJEUFiles{},
    fCollidingSystem{Form("PbPb")}, fCollidingEnergyGeV{5020},
    fYearOfDataTaking{2018}, fDoJetPtSmearing{kFALSE}, 
    fFixJetArrays{kFALSE}, fEventCut{nullptr}, fJetCut{nullptr} {
...
void ForestAODReader::setupBranches() {
    ...
    if ( fUseCaloJetBranch) fCaloJetTree->SetBranchStatus("*", 1);
    if ( fUsePartFlowJetBranch ) fPartFlowJetTree->SetBranchStatus("*", 1);
    ...
    if ( fUseHltBranch ) {
        for (std::list<TString>::iterator it = fHltFilterName.begin(); 
                it != fHltFilterName.end(); it++) {
            //std::string str = *it->Data();
            fHltTree->SetBranchStatus(Form( "%s", *it->Data() ), 1);
            //fHltTree->SetBranchStatus(Form( "%s", str.c_str() ), 1);
        }
    } // if ( fUseHltBranch ) ß
...
Event* ForestAODReader::returnEvent() {
    ...
    // Fill HLT branche
    if ( fUseHltBranch ) {
        fEvent->setJetTriggerBit( fJetTriggerBit );
    }
